...
- 🟢 **Manager/hot-manager execution** — manager and routine-safe hot-manager deploy/pull remain available for primary and approved-secondary SPL token-account positions. An opt-in `StrategyPolicy` additionally lets those roles execute one external venue CPI only after reconstructing a curator-committed Merkle leaf. Generic CPI cannot write vault-owned token accounts; those movements require a typed adapter with post-call reconciliation.
...
- 🟢 **Typed SPL token-balance adapter** — Phase 3 includes a separately domain-separated Merkle leaf for canonical primary and approved-secondary reserve/position movement. The dynamic amount is capped by a proof-bound maximum; execution requires stored/on-chain position agreement, exact opposing token deltas, unchanged share supply, rolling-limit consumption, and `Position`/`VaultAsset` ledger reconciliation. It is a reference custody adapter, not an external lending/swap/LP adapter.
...
- 🟢 **Adapter isolation from generic CPI** — the generic Merkle path rejects every writable SPL Token or Token-2022 account controlled by the vault PDA. Typed adapters use unique leaf domains and explicit canonical-account, amount, balance-delta, and accounting rules. New protocols should add new typed adapters instead of weakening this guard.
