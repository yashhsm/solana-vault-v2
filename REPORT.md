...
NAV, protocol-specific lending/swap/LP adapters, external protocol custody,
tranche-aware or secondary-asset instant settlement, oracle adapters,
tranche-aware performance fees, and global protocol fee bps overrides are not
implemented. Generic Merkle-authorized CPI and a typed token-balance reference
adapter exist, but neither claims arbitrary external-protocol accounting.
...
| Phase 3 venue registry         | Partial: `withdraw_assets` is disabled by default and gated by the creation-time `ExternallyManagedWithdrawals` TLV extension plus active matching venue approval and recipient authority. Strategist-bound Merkle roots authorize selected vault-signed CPI shapes, while generic CPI rejects writable vault-owned token accounts. A typed token-balance reference adapter proof-binds canonical primary/secondary reserve and position accounts, action, policy version, and per-call maximum; it checks exact deltas and reconciles `Position`/`VaultAsset` ledgers. Protocol-specific external custody adapters and vault-in-vault cycle prevention remain future work.                                                                                                                                                                                                                                                                         |
...
- Generic venue CPI requires an active strategist policy proof, matching venue
  records, bounded operators, and reentrancy/share-supply guards. It cannot
  write a vault-owned token account; custody movement requires a typed adapter.
