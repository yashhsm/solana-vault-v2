...
#[derive(Accounts)]
pub struct DeployVenuePosition<'info> {
...
impl<'info> DeployVenuePosition<'info> {
    fn transfer_to_position(&self, amount: u64) -> Result<()> {
        let seeds: &[&[&[u8]]] = &[&[
            VAULT_CONFIG_SEED,
...
pub fn handler(ctx: Context<DeployVenuePosition>, amount: u64) -> Result<()> {
    ...
    let secondary_ledger = if is_primary_asset {
        ...
        require!(
            vault_asset.idle_balance >= amount,
    ...
    ctx.accounts.transfer_to_position(amount)?;
